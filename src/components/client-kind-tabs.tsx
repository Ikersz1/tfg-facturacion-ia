...
type Props = { activeKind: ClientKind };

export function ClientKindTabs({ activeKind }: Props) {
  const tab = (kind: ClientKind, label: string) => {
    const active = activeKind === kind;
    return (
      <Link
        href={`/clients?kind=${kind}`}
  ...
  return (
    ...
    >
      {tab("company", "Empresas")}
      {tab("individual", "Particulares")}
    </nav>
  );
}
