  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-3 border-t border-zinc-200 px-4 py-3 dark:border-zinc-700">
      <p className="text-xs text-zinc-500 dark:text-zinc-400">
        {totalItems} {totalItems === 1 ? "factura" : "facturas"} · página {page} de {totalPages}
      </p>
      <div className="flex items-center gap-2">
        {prevPage ? (
          <Link href={buildInvoicesListPageUrl(filters, prevPage)} className={navBtn}>
            Anterior
          </Link>
        ) : (
        ...
        {nextPage ? (
          <Link href={buildInvoicesListPageUrl(filters, nextPage)} className={navBtn}>
            Siguiente
          </Link>
        ) : (
          <span className={navBtnDisabled} aria-disabled>
